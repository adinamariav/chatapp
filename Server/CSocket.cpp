...
void CSocket::initDescriptor(){
    socketDescriptor = socket(AF_INET, SOCK_STREAM, 0); //socket system call
...
void CSocket::initAddress(string IPaddress, int portNumber){
    ...
    socketAddress.sin_port=htons(portNumber); //it is necessary to convert this integer from host byte order to network byte order
    socketAddress.sin_addr.s_addr=inet_addr((IPaddress.c_str())); //function from arpa/inet.h - the IP address of the machine on which the server is running, INADDR_ANY - constant that gets this address
}
